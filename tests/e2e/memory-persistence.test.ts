...
test.describe('Memory & Persistence E2E Tests', () => {
  ...
  test.describe('Chat Session Persistence', () => {
    test('should persist chat across page reloads', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="message-content"]', { timeout: 10000 });
      ...
      expect(assistantMessages.some(m => m.content === originalResponse)).toBe(true);
    });

    test('should maintain conversation context after reload', async ({ page }) => {
      await chatPage.createNewChat();
      
      // Establish context
      await chatPage.sendUserMessage('My name is TestUser and I work with RoboRail maintenance');
      await chatPage.isGenerationComplete();
  ...
  test.describe('User Memory Across Sessions', () => {
    ...
    test('should remember user information across login sessions', async ({ page }) => {
      // First session
      await authPage.login(testUser.email, testUser.password);
      await page.waitForURL('/');
      
      await chatPage.createNewChat();
      await chatPage.sendUserMessage('My name is TestUser and I prefer morning shifts');
      await chatPage.isGenerationComplete();
      
      await chatPage.sendUserMessage('What did I tell you about my shift preference?');
      await chatPage.isGenerationComplete();
      ...
      // Create new chat
      await chatPage.createNewChat();
      await chatPage.sendUserMessage('What shift preference did I mention before?');
      await chatPage.isGenerationComplete();
    ...
    test('should maintain chat history for authenticated users', async ({ page }) => {
      await authPage.login(testUser.email, testUser.password);
      await page.waitForURL('/');
      
      // Create first chat
      await chatPage.createNewChat();
      await chatPage.sendUserMessage('First chat message about safety protocols');
      await chatPage.isGenerationComplete();
      ...
      expect(messages.some(m => m.content?.includes('safety protocols'))).toBe(true);
      
      await page.goto(secondChatUrl);
      await page.waitForSelector('[data-testid="message-content"]', { timeout: 10000 });
      
      messages = await chatPage.getAllMessages();
      expect(messages.some(m => m.content?.includes('maintenance'))).toBe(true);
    });
  });
  ...
  test.describe('Chat History Management', () => {
    ...
    test('should allow navigation between chat histories', async ({ page }) => {
      ...
      if (await chatHistoryItems.count() > 1) {
        ...
        await page.waitForTimeout(1000);
        const messages = await chatPage.getAllMessages();
        expect(messages.some(m => m.content?.includes('First chat content'))).toBe(true);
      }
    });
  ...
  test.describe('Data Persistence Edge Cases', () => {
    test('should handle very long conversations', async ({ page }) => {
      ...
      for (let i = 0; i < 10; i++) {
        await chatPage.sendUserMessage(`Message ${i + 1}: Testing long conversation persistence`);
        await chatPage.isGenerationComplete();
      ...
      expect(userMessages.some(m => m.content?.includes('Message 1:'))).toBe(true);
      expect(userMessages.some(m => m.content?.includes('Message 10:'))).toBe(true);
    });

    test('should handle special characters in messages', async ({ page }) => {
      await chatPage.createNewChat();
      
      const specialMessage = 'Test with émojis 🤖, spëcial chäracters, and "quotes" & symbols: @#$%^&*()';
    ...
    test('should handle concurrent chat sessions', async ({ browser }) => {
      ...
      expect(messages1.some(m => m.content?.includes('context 1'))).toBe(true);
      expect(messages1.some(m => m.content?.includes('context 2'))).toBe(false);
      
      expect(messages2.some(m => m.content?.includes('context 2'))).toBe(true);
      expect(messages2.some(m => m.content?.includes('context 1'))).toBe(false);
  ...
  test.describe('Memory Performance', () => {
    test('should load chat history efficiently', async ({ page }) => {
      ...
      await chatPage.sendUserMessage('Tell me everything about RoboRail safety procedures in detail');
      await chatPage.isGenerationComplete();
