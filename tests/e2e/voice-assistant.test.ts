...
test.describe('Voice Assistant E2E Tests', () => {
  ...
  test.describe('Voice Interface', () => {
    ...
    test('should handle microphone permissions', async ({ page, browserName }) => {
      // Skip on webkit due to permission handling differences
      test.skip(browserName === 'webkit', 'Webkit handles permissions differently');
    ...
    test('should handle microphone permission denial', async ({ page, browserName }) => {
      test.skip(browserName === 'webkit', 'Webkit handles permissions differently');
  ...
  test.describe('Voice Recording', () => {
    test('should start and stop voice recording', async ({ page, browserName }) => {
      test.skip(browserName === 'webkit', 'Voice recording may not work reliably in webkit');
    ...
    test('should show voice waveform during recording', async ({ page, browserName }) => {
      test.skip(browserName === 'webkit', 'Voice features may not work in webkit');
    ...
    test('should handle voice recording timeout', async ({ page, browserName }) => {
      test.skip(browserName === 'webkit' || process.env.CI === 'true', 'Long timeout test - skip in CI');
  ...
  test.describe('Voice to Text', () => {
    test.skip('should convert voice to text and send message', async ({ page, browserName }) => {
      // This test is complex because it requires actual audio input
      // Skipping for now as it would need mock audio data
      
      test.skip(true, 'Voice-to-text requires complex audio mocking - implement when needed');
    ...
    test('should show transcription in progress', async ({ page, browserName }) => {
      test.skip(browserName === 'webkit', 'Voice features may not work in webkit');
      ...
      const transcriptionIndicator = page.getByTestId('voice-transcription-indicator');
  ...
  test.describe('Voice Error Handling', () => {
    test('should handle network errors during voice processing', async ({ page, browserName }) => {
      test.skip(browserName === 'webkit', 'Voice features may not work in webkit');
    ...
    test('should handle audio processing errors', async ({ page, browserName }) => {
      test.skip(browserName === 'webkit', 'Voice features may not work in webkit');
  ...
  test.describe('Accessibility', () => {
    ...
    test('should announce voice recording state to screen readers', async ({ page, browserName }) => {
      test.skip(browserName === 'webkit', 'Voice features may not work in webkit');
      ...
      if (await voiceButton.count() > 0) {
        ...
        const statusAnnouncement = page.getByTestId('voice-status-announcement');
  ...
  test.describe('Voice Integration with Chat', () => {
    test('should work seamlessly with text chat', async ({ page, browserName }) => {
      test.skip(browserName === 'webkit', 'Voice features may not work in webkit');
    ...
    test('should preserve conversation context with voice messages', async ({ page }) => {
