...
test.describe('RAG Document Upload E2E Tests', () => {
  ...
  test.describe('Document Upload Functionality', () => {
    test('should upload and process document successfully', async ({
      ...
      await expect(page.locator('[data-testid="document-item"]')).toContainText(
        'roborail-safety-manual.txt',
      );
    });
  ...
  test.describe('RAG Query Functionality', () => {
    test('should use uploaded document knowledge in chat responses', async ({
      page,
    }) => {
      ...
      await chatPage.sendUserMessage(
        'What does error code E001 mean and how do I fix it?',
      );
    ...
    test('should combine RAG knowledge with general AI knowledge', async () => {
      await chatPage.sendUserMessage(
        'Tell me about RoboRail safety and also general industrial safety practices',
      );
      await chatPage.isGenerationComplete();
  ...
  test.describe('Document Management', () => {
    ...
    test('should delete documents', async ({ page }) => {
      ...
      const deleteButton = page
        .locator('[data-testid="delete-document-button"]')
        .first();
      ...
      ).not.toContainText('delete-test.txt');
    });
    ...
    test('should show document processing status', async ({ page }) => {
      ...
      await expect(statusElement).toContainText(
        /processing|analyzing|completed/i,
      );
    });
  });
  ...
  test.describe('Integration with Chat', () => {
    test('should indicate when RAG knowledge is used in response', async () => {
      await chatPage.sendUserMessage(
        'What safety equipment is required for RoboRail?',
      );
      await chatPage.isGenerationComplete();
      ...
      expect(assistantMessage.content.toLowerCase()).toMatch(
        /safety|protective|equipment|glasses|boots|helmet/,
      );

      // Check if there's an indicator that RAG knowledge was used
      // This depends on the specific implementation
      const ragIndicator = assistantMessage.element.locator(
        '[data-testid="rag-source-indicator"]',
      );
    ...
    test('should handle follow-up questions using context', async () => {
      ...
      await chatPage.sendUserMessage(
        'Tell me about RoboRail emergency procedures',
      );
      await chatPage.isGenerationComplete();

      let assistantMessage = await chatPage.getRecentAssistantMessage();
      expect(assistantMessage.content.toLowerCase()).toMatch(
        /emergency|stop|evacuate/,
      );
