...
test.describe('Integration Scenarios E2E', () => {
  ...
  test.describe('End-to-End RoboRail Workflows', () => {
    test('should complete full RoboRail troubleshooting workflow', async ({ page }) => {
      await chatPage.createNewChat();
      
      // Step 1: User reports an issue
      await chatPage.sendUserMessage('My RoboRail system is showing error code E001 and making unusual noises');
      await chatPage.isGenerationComplete();
      
      let assistantMessage = await chatPage.getRecentAssistantMessage();
      expect(assistantMessage.content.toLowerCase()).toMatch(/error|e001|troubleshoot/);
      
      // Step 2: Assistant requests more information
      await chatPage.sendUserMessage('The noise started about 2 hours ago, and the system stopped moving');
      await chatPage.isGenerationComplete();
      ...
      assistantMessage = await chatPage.getRecentAssistantMessage();
      expect(assistantMessage.content.toLowerCase()).toMatch(/check|inspect|safety|emergency/);
      
      // Step 4: User follows guidance
      await chatPage.sendUserMessage('I checked the emergency stops and they are working. What next?');
      await chatPage.isGenerationComplete();
    ...
    test('should handle complete maintenance planning workflow', async ({ page }) => {
      await chatPage.createNewChat();
      
      // Maintenance planning scenario
      await chatPage.sendUserMessage('I need to plan routine maintenance for our RoboRail system. When should I do this?');
      await chatPage.isGenerationComplete();
      
      let assistantMessage = await chatPage.getRecentAssistantMessage();
      expect(assistantMessage.content.toLowerCase()).toMatch(/maintenance|schedule|routine/);
      ...
      await chatPage.sendUserMessage('What safety precautions should I take during maintenance?');
      await chatPage.isGenerationComplete();
      
      assistantMessage = await chatPage.getRecentAssistantMessage();
      expect(assistantMessage.content.toLowerCase()).toMatch(/safety|ppe|protective|equipment/);
      
      // Documentation
      await chatPage.sendUserMessage('How should I document the maintenance work?');
      await chatPage.isGenerationComplete();
      
      assistantMessage = await chatPage.getRecentAssistantMessage();
      expect(assistantMessage.content.toLowerCase()).toMatch(/document|record|log/);
    });
    ...
    test('should handle emergency response workflow', async ({ page }) => {
      ...
      // Emergency scenario
      await chatPage.sendUserMessage('URGENT: RoboRail system has stopped suddenly and there\'s a loud alarm. What should I do immediately?');
      await chatPage.isGenerationComplete();
      
      let assistantMessage = await chatPage.getRecentAssistantMessage();
      expect(assistantMessage.content.toLowerCase()).toMatch(/emergency|stop|alarm|safety|evacuate/);
      
      // Response to emergency instructions
      await chatPage.sendUserMessage('I pressed the emergency stop button and cleared the area. The alarm is still sounding.');
      await chatPage.isGenerationComplete();
      ...
      // Next steps
      await chatPage.sendUserMessage('Who should I contact and what information do they need?');
      await chatPage.isGenerationComplete();
      
      assistantMessage = await chatPage.getRecentAssistantMessage();
      expect(assistantMessage.content.toLowerCase()).toMatch(/contact|maintenance|team|information/);
    });
  });
  ...
  test.describe('Multi-User Collaboration Scenarios', () => {
    ...
    test('should handle shift handover scenario', async ({ browser }) => {
      ...
      await operatorChat.sendUserMessage('Documenting shift report: RoboRail operated normally for first 6 hours, minor calibration adjustment made at 14:30');
      ...
      await supervisorChat.sendUserMessage('What should I review when taking over a shift for RoboRail operations?');
      await supervisorChat.isGenerationComplete();
      
      const assistantMessage = await supervisorChat.getRecentAssistantMessage();
      expect(assistantMessage.content.toLowerCase()).toMatch(/shift|review|operations|status/);
  ...
  test.describe('Real-World Usage Patterns', () => {
    test('should handle typical daily usage pattern', async ({ page }) => {
      ...
      await chatPage.sendUserMessage('Good morning! I\'m starting my shift. What\'s the startup checklist for RoboRail?');
      await chatPage.isGenerationComplete();
      
      let assistantMessage = await chatPage.getRecentAssistantMessage();
      expect(assistantMessage.content.toLowerCase()).toMatch(/startup|checklist|morning/);
      
      // Mid-shift operational questions
      await chatPage.sendUserMessage('The system is running but speed seems lower than usual. Normal parameters?');
      await chatPage.isGenerationComplete();
      ...
      await chatPage.sendUserMessage('My shift is ending. What shutdown procedures should I follow?');
      await chatPage.isGenerationComplete();
      
      assistantMessage = await chatPage.getRecentAssistantMessage();
      expect(assistantMessage.content.toLowerCase()).toMatch(/shutdown|end|shift|procedures/);
    ...
    test('should handle complex multi-topic conversation', async ({ page }) => {
      ...
      // Topic 1: Safety
      await chatPage.sendUserMessage('Explain the safety protocols for RoboRail maintenance');
      await chatPage.isGenerationComplete();
      
      // Topic 2: Technical issue
      await chatPage.sendUserMessage('Now, separately, how do I calibrate the position sensors?');
      await chatPage.isGenerationComplete();
      
      // Topic 3: Documentation
      await chatPage.sendUserMessage('And what\'s the proper way to document these procedures?');
      await chatPage.isGenerationComplete();
      
      // Reference previous topics
      await chatPage.sendUserMessage('If I find a safety issue during sensor calibration, what\'s the protocol?');
      await chatPage.isGenerationComplete();
      
      const assistantMessage = await chatPage.getRecentAssistantMessage();
      // Should reference both safety protocols and calibration context
      expect(assistantMessage.content.toLowerCase()).toMatch(/safety|calibration|protocol/);
    });
  });
  ...
  test.describe('Error Recovery and Resilience', () => {
    test('should recover from temporary service interruptions', async ({ page }) => {
      ...
      // Normal operation
      await chatPage.sendUserMessage('What are the daily operational checks for RoboRail?');
      await chatPage.isGenerationComplete();
      ...
      await chatPage.sendUserMessage('Can you repeat the last information about daily checks?');
      await chatPage.isGenerationComplete();
      
      assistantMessage = await chatPage.getRecentAssistantMessage();
      expect(assistantMessage.content.toLowerCase()).toMatch(/daily|check|operational/);
    });
    ...
    test('should handle concurrent user sessions gracefully', async ({ browser }) => {
      ...
      await Promise.all(
        chatPages.map(chatPage => chatPage.createNewChat())
      );
  ...
  test.describe('Performance and Scalability', () => {
    ...
    test('should maintain responsiveness with long conversation history', async ({ page }) => {
      await chatPage.createNewChat();
      
      // Build up conversation history
      for (let i = 1; i <= 8; i++) {
        await chatPage.sendUserMessage(`Message ${i}: Tell me about RoboRail topic ${i}`);
        await chatPage.isGenerationComplete();
      ...
      await chatPage.sendUserMessage('Final question: Summarize what we discussed');
      await chatPage.isGenerationComplete();
  ...
  test.describe('Accessibility and Usability', () => {
    ...
    test('should work with high contrast and accessibility features', async ({ page }) => {
      // Simulate high contrast mode
      await page.emulateMedia({ colorScheme: 'dark' });
      
      await chatPage.createNewChat();
      await chatPage.sendUserMessage('Testing accessibility in high contrast mode');
      await chatPage.isGenerationComplete();
