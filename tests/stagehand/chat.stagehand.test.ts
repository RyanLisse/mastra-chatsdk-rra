...
test.describe.skip(!stagehandAvailable, "RoboRail Assistant Chat Tests", () => {
  ...
  test.describe("RoboRail Specific Features", () => {
    test("should handle RoboRail maintenance queries", async () => {
      ...
      expect(response.content.toLowerCase()).toMatch(/maintenance|service|inspect|clean|check/);
    }, 60000);
    ...
    test("should handle error code troubleshooting", async () => {
      ...
      expect(response.content.toLowerCase()).toMatch(/error|fix|troubleshoot|solution|check/);
    }, 60000);
  });
  ...
  test.describe("UI/UX Features", () => {
    ...
    test("should display typing indicators during response generation", async () => {
      ...
      await stagehand.act('Type "Explain RoboRail safety protocols" and send it');
