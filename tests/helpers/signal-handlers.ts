 ...
 */
export function removeShutdownCallback(
  callback: () => Promise<void> | void,
): void {
...
export async function gracefulShutdown(
  ...
  try {
    ...
    if (postHealth.details.connections === 0) {
      ...
      if (finalHealth.details.connections === 0) {
        ...
        console.error(
          `   ❌ ${finalHealth.details.connections} connections still active after force cleanup`,
        );
        exitCode = 1;
      }
    }
    ...
    console.log(
      `🎉 ${context} graceful shutdown completed in ${shutdownTime}ms`,
    );
  } catch (error) {
  ...
  if (totalShutdownTime > maxShutdownTime) {
    console.warn(
      `⚠️  ${context} shutdown took ${totalShutdownTime}ms (exceeded ${maxShutdownTime}ms limit)`,
    );
  }
...
export function resetShutdownState(): void {
  if (process.env.NODE_ENV !== 'test' && process.env.PLAYWRIGHT !== 'true') {
    console.warn(
      '⚠️  resetShutdownState() should only be used in test environments',
    );
    return;
  }
