...
import { registerTestSignalHandlers, addShutdownCallback } from './helpers/signal-handlers';
...
afterAll(async () => {
  ...
  try {
    ...
    if (health.details.connections > 0) {
      console.log(`📊 Found ${health.details.connections} active connections: ${health.details.names.join(', ')}`);
    }
    ...
    if (postCleanupHealth.details.connections > 0) {
      console.warn(`⚠️  Warning: ${postCleanupHealth.details.connections} connections still active after cleanup`);
    } else {
