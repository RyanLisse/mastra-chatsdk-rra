...
import { validateTestDatabaseConfig, runTestMigrations, createTestDatabase } from '../lib/db/test-config';
...
async function globalSetup(config: FullConfig) {
  ...
  try {
    ...
    console.log(`   📊 Database contains: ${stats[0].users} users, ${stats[0].chats} chats, ${stats[0].messages} messages`);
    ...
    if (error instanceof Error && error.message?.includes('connect')) {
      ...
      console.error('   1. Check your .env.test file has the correct POSTGRES_URL');
      console.error('   2. Ensure your test database/branch is accessible');
      console.error('   3. Verify your database credentials');
      console.error('   4. Run: bun run db:test:validate for detailed diagnosis');
