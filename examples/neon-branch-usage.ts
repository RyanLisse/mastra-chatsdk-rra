...
async function demonstrateNeonBranchUsage() {
  ...
  try {
    ...
    console.log(
      `📊 Configuration: ${testDb.config.isTestBranch ? 'Neon Test Branch' : 'Standard Database'}`,
    );
    ...
    if (testDb.createTestBranch && testDb.cleanupTestBranches) {
      ...
      console.log(
        `Found ${existingTestBranches.length} existing test branches:`,
        existingTestBranches,
      );
      ...
      console.log(
        `🔗 Connection: ${tempBranch.connectionString.substring(0, 50)}...`,
      );
