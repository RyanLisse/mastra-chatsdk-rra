...
} from '../lib/ai/env-validation';
import { getProviderEnvironment } from '../lib/ai/provider-config';
...
async function main() {
  ...
  console.log(
    `Validation Status: ${status.isValid ? '✅ Valid' : '❌ Invalid'}`,
  );
  ...
  for (const provider of status.availableProviders) {
    ...
    for (const model of providerModels) {
      ...
      console.log(
        `  • ${model.name.padEnd(20)} ${tier} - ${model.description}`,
      );
    }
  }
  ...
  if (status.availableProviders.length === 0) {
    console.log(
      '  🚨 No providers configured! Set at least OPENAI_API_KEY to get started.',
    );
  } else if (status.availableProviders.length < 2) {
    console.log(
      '  💡 Consider adding more providers for better model variety and redundancy.',
    );
  } else if (status.availableProviders.length === 4) {
    console.log(
      '  🎉 Excellent! All providers are configured for maximum model variety.',
    );
  }

  if (!status.availableProviders.includes('openai')) {
    console.log(
      '  ⚠️  OpenAI is the primary provider - consider adding it for best reliability.',
    );
  }
  ...
  console.log(
    '  • Get Google key:      https://aistudio.google.com/app/apikey',
  );
