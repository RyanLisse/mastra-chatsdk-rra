...
console.log(`${chatModels.length === 23 ? '✅' : '❌'} Count verification: ${chatModels.length === 23 ? 'PASSED' : 'FAILED'}\n`);
...
Object.keys(providers).forEach((providerId) => {
  ...
  console.log(`${hasModels ? '✅' : '❌'} ${providers[provider].name}: ${hasModels ? 'PRESENT' : 'MISSING'}`);
});

console.log(`${uniqueProviders.size === 4 ? '✅' : '❌'} Provider verification: ${uniqueProviders.size === 4 ? 'PASSED' : 'FAILED'}\n`);
...
Object.entries(expectedCounts).forEach(([provider, expectedCount]) => {
  ...
  console.log(`${passed ? '✅' : '❌'} ${providers[provider as Provider].name}: Expected ${expectedCount}, got ${actualCount}`);
});

console.log(`${distributionPassed ? '✅' : '❌'} Distribution verification: ${distributionPassed ? 'PASSED' : 'FAILED'}\n`);

// 4. Tier Distribution
console.log('🏆 Model Tier Distribution:');
const tierCounts = chatModels.reduce((acc, model) => {
...
console.log(`${tiersPassed ? '✅' : '❌'} Tier distribution: ${tiersPassed ? 'PASSED' : 'FAILED'}\n`);
...
const capabilitiesPassed = capabilities.vision > 0 && capabilities.reasoning > 0 && capabilities.tools > 0;
console.log(`${capabilitiesPassed ? '✅' : '❌'} Capability distribution: ${capabilitiesPassed ? 'PASSED' : 'FAILED'}\n`);
...
Object.entries(entitlementsByUserType).forEach(([userType, entitlements]) => {
  ...
  console.log(`✅ ${userType.toUpperCase()}: ${availableCount} models (${validModels} valid)`);
  
  if (availableCount !== validModels) {
    console.log(`❌ Some model IDs in ${userType} entitlements don't exist in chatModels`);
  }
});
...
console.log(`✅ Claude 4 models: ${claude4Models.length} (${claude4Models.map(m => m.name).join(', ')})`);

// Gemini 2.5 models  
const gemini25Models = chatModels.filter(m => m.id.includes('gemini-2.5'));
console.log(`✅ Gemini 2.5 models: ${gemini25Models.length} (${gemini25Models.map(m => m.name).join(', ')})`);

// o3 models
const o3Models = chatModels.filter(m => m.id.includes('o3'));
console.log(`✅ o3 models: ${o3Models.length} (${o3Models.map(m => m.name).join(', ')})`);

// LLaMA models
const llamaModels = chatModels.filter(m => m.id.includes('llama'));
console.log(`✅ LLaMA models: ${llamaModels.length} (${llamaModels.map(m => m.name).join(', ')})`);

// 8. Context Window Verification
console.log('\n💾 Context Window Analysis:');
const contextWindows = chatModels.map(m => m.capabilities.contextWindow).sort((a, b) => b - a);
console.log(`✅ Largest context window: ${contextWindows[0].toLocaleString()} tokens`);
console.log(`✅ Smallest context window: ${contextWindows[contextWindows.length - 1].toLocaleString()} tokens`);

const megaTokenModels = chatModels.filter(m => m.capabilities.contextWindow >= 1000000);
console.log(`✅ Models with 1M+ tokens: ${megaTokenModels.length} (${megaTokenModels.map(m => m.name).join(', ')})`);
...
console.log(`${passedChecks === totalChecks ? '🎉 ALL VERIFICATIONS PASSED!' : '❌ Some verifications failed'}`);
...
Object.entries(providers).forEach(([providerId, providerData]) => {
  ...
  if (sampleModel) {
    console.log(`  ${providerData.name}: ${sampleModel.name} (${sampleModel.tier}, ${sampleModel.capabilities.contextWindow.toLocaleString()} tokens)`);
  }
});
