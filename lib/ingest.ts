 ...
 */
export function chunkText(text: string, chunkSize = 250, overlap = 50): string[] {
 ...
 */
export async function storeChunks(chunks: string[], embeddings: number[][]): Promise<number> {
...
export async function ingestDocuments(customPath?: string): Promise<{
  ...
  try {
    ...
    const docPath = customPath || path.join(process.cwd(), 'knowledge-base', 'robo-manual.md');
    ...
    console.log(`Document read successfully. Length: ${document.length} characters`);
