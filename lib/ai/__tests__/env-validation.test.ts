  ...
  describe('validateEnvironment', () => {
    ...
    it('should reject invalid OpenAI API key format', () => {
      ...
      expect(status.invalidKeys).toContainEqual({
        provider: 'openai',
        reason:
          'Invalid format. Should start with "sk-" followed by alphanumeric characters',
      });
    });
    ...
    it('should reject invalid Anthropic API key format', () => {
      ...
      expect(status.invalidKeys).toContainEqual({
        provider: 'anthropic',
        reason:
          'Invalid format. Should start with "sk-ant-" followed by alphanumeric characters',
      });
    });

    it('should validate Google API key format', () => {
      process.env.GOOGLE_API_KEY =
        'AIzaSyAbc123456789012345678901234567890123456';
    ...
    it('should reject invalid Google API key format', () => {
      ...
      expect(status.invalidKeys).toContainEqual({
        provider: 'google',
        reason:
          'Invalid format. Should start with "AIza" followed by alphanumeric characters',
      });
    });
    ...
    it('should reject invalid Groq API key format', () => {
      ...
      expect(status.invalidKeys).toContainEqual({
        provider: 'groq',
        reason:
          'Invalid format. Should start with "gsk_" followed by alphanumeric characters',
      });
    });
    ...
    it('should provide warnings for missing providers', () => {
      ...
      expect(status.warnings).toContain(
        'ANTHROPIC API key not found. Set ANTHROPIC_API_KEY to enable anthropic models.',
      );
      expect(status.warnings).toContain(
        'GOOGLE API key not found. Set GOOGLE_API_KEY to enable google models.',
      );
      expect(status.warnings).toContain(
        'GROQ API key not found. Set GROQ_API_KEY to enable groq models.',
      );
    });
