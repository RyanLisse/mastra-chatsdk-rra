...
export class NeonBranchManager {
  ...
  async listBranches(projectId?: string): Promise<NeonBranch[]> {
    ...
    if (!pid) {
      throw new Error(
        'Project ID is required. Provide it as parameter or set defaultProjectId',
      );
    }
  ...
  async createBranch(options: CreateBranchOptions): Promise<NeonBranch> {
    ...
    if (!pid) {
      throw new Error(
        'Project ID is required. Provide it in options or set defaultProjectId',
      );
    }
    ...
    if (!options.name.includes('test') && !options.name.startsWith('test-')) {
      console.warn(
        `⚠️  Branch name "${options.name}" doesn't indicate it's for testing`,
      );
    }
  ...
  async getBranchConnectionString(
    ...
    try {
      ...
      throw this.handleNeonError(
        `get connection string for branch ${branchName}`,
        error,
      );
    }
  }
  ...
  async createTempTestDatabase(
    ...
    const connectionString = await this.getBranchConnectionString(
      branch.name,
      projectId,
    );
 ...
 */
export function createNeonBranchManager(
  config: NeonBranchManagerConfig = {},
): NeonBranchManager {
 ...
 */
export function extractProjectIdFromConnectionString(
  connectionString: string,
): string | null {
  try {
 ...
 */
export function extractBranchNameFromConnectionString(
  connectionString: string,
): string | null {
  try {
    const url = new URL(connectionString);
